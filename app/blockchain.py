...
class Blockchain:
    ...
    def chain_is_valid(self, chain):
        ...
        for i in range(1, len(chain)):
            ...
            print(block_hash)
            if i != 1 or block_hash[:4] != '0000': #채우시오 : i가 1이 아니거나 hash값 찾은 hash값 조건 넣기 :
                validChain = False
    ...
    def create_merkle_tree(self, transactions):
        if len(transactions) == 0:
            ...
            transactions.append(transactions[-1]) # tx가 하나니까 있는 tx 트랜잭션 리스트에 넣기. 가장 마지막 tx를 trasactions에 추가
            new_level = []
            ...
            while len(transactions) > 1:
                if len(transactions) % 2 == 1:  #채우시오 : 홀수일 때:
